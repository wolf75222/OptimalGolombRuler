...
Cette sélection permet d'évaluer l'impact de la granularité MPI/OpenMP sur les performances tout en utilisant le même nombre total de workers.

\subsection{Analyse des patterns de communication}

L'overhead de communication dépend du pattern utilisé :
\begin{itemize}
    \item \textbf{Hypercube (V1, V2)} : $O(\log P)$ étapes de communication. Chaque processus communique avec $\log_2(P)$ voisins pour propager le meilleur bound.
    \item \textbf{Allreduce (V3)} : Utilise \texttt{MPI\_Allreduce} avec \texttt{MPI\_MIN}. Optimisé par la bibliothèque MPI, généralement $O(\log P)$ également mais avec moins de code applicatif.
\end{itemize}
...
\begin{important}{Benchmarks MPI en cours}
Les résultats MPI détaillés seront ajoutés dès la fin des jobs sur le cluster Romeo. L'analyse ci-dessous est théorique.
\end{important}
...
\begin{enumerate}
    \item \textbf{Mémoire distribuée} : Chaque processus possède son espace mémoire, éliminant les contentions de cache L3 partagé.
    \item \textbf{Scaling multi-nœuds} : Possibilité d'utiliser 384 cœurs (2 nœuds) au lieu de 192 (1 nœud).
\end{enumerate}

Théoriquement, la configuration 2 MPI $\times$ 96 threads devrait offrir les meilleures performances car elle minimise les communications tout en bénéficiant de la localité NUMA.
