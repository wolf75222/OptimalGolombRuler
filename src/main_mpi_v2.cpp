...
int main(int argc, char* argv[])
{
    ...
    if (rank == 0) {
        std::cout << "===========================================" << std::endl;
        std::cout << " GOLOMB RULER SEARCH - MPI V2 (Hypercube + BitSet128)" << std::endl;
        std::cout << "===========================================" << std::endl;
    ...
    searchGolombMPI_V2(n, maxLen, best, hypercube);

    MPI_Barrier(MPI_COMM_WORLD);
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = end - start;

    long long exploredCount = getExploredCountMPI_V2();
